...
def generate_figure(original: Graph, regions: List[Graph], edge_data: List[str]) -> None:
    ...
    gs = gridspec.GridSpec(2, len(regions), height_ratios=[3, 1], width_ratios=[0.5])
    # Top subplot that spans all columns
    axes = [fig.add_subplot(gs[0, :])]  # Full row (row 0, all columns)
    # Bottom subplots
    for idx, _ in enumerate(regions):
        sub_axes = fig.add_subplot(gs[1, idx])
        axes.append(sub_axes)
    # Plot each graph
    for i, (ax, current_graph) in enumerate(zip(axes, [original] + regions)):
        # Render CI
        ...
        if i != 0:
            ...
            print(len(coords) != 0, current_graph.road_network.name)
            if len(coords) == 0:
                quit()
...
def generate_figure2(original: Graph, regions: List[Graph], edge_data: List[str]) -> None:
    """
    :param original: Original graph
    :param regions: 3 regions of the original graph
    :param edge_data: edge data (original, planned)
    ...
    for i, (ax, current_graph) in enumerate(zip([ax1, ax2, ax3, ax4], [original] + regions)):
        ...
        visualizer.plot_ci_diff(planned_array, orig_array, current_graph, axes=ax)
        ax.autoscale_view(True, True, True)
        ax.set_title(current_graph.road_network.name, fontsize=15)
        # Arrows & rectangles for sub regions
        if i != 0:
            coords: np.ndarray = np.array([(junction.x, junction.y) for junction in current_graph.road_network.get_junctions_list()])
            x, y = np.mean(coords, axis=0)
...
if __name__ == '__main__':
    ...
    data: List[str] = [
        DirPaths.SCENARIO_STATISTICS.format("itsc_25200_32400_planned3") + "/itsc_25200_32400_planned3_edgeData.out.xml",
        DirPaths.SCENARIO_STATISTICS.format("itsc_25200_32400") + "/stats_edgedata.out.xml"
    ]
    generate_figure2(graph, regions, data)
