...
from psutil import Process, cpu_count, NoSuchProcess
...
class TaskManager:
    ...
    def call_shell_block(command: str, cwd: str = None, message: bool = True) -> Tuple[bool, int]:
        ...
        """
        if message:
            print(f"Calling command: '{cmd_split(command, posix=False)}' cwd: {cwd}")
            print(f"On process: {current_process().name}")
        success: bool = False
        ret_val: int = -1
        try:
            ret_val = call(cmd_split(command, posix=False), stdout=DEVNULL, stdin=DEVNULL, cwd=cwd)
            success = True
        except SubprocessError as e:
            print(f"Error:! {e}")
        if message:
