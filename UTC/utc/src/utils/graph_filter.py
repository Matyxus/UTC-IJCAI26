...
if __name__ == "__main__":
    ...
    scenario_name: str = "lust_25200_32400_planned_old_br_mip"
    # for color in ["_lime", "_red", "_orange"]:
    tmp.filter_scenario(scenario_name, f"lust_red", f"lust_25200_32400_br_red_filtered_old_mip", full_path=False)
        # tmp.filter_planned(scenario_name + color + "_filtered", scenario_name + color + "_planned", f"DCC{color}")
    # scenario_name = "itsc_25200_32400"
    # tmp.filter_scenario(scenario_name, "DCC_central", scenario_name + "_central_filtered", )

    # for color in ["_green", "_red", "_orange"]:
    #    tmp.filter_scenario(scenario_name + color + "_planned", f"DCC{color}", f"{scenario_name + color}_pddl_filtered", full_path=False)
