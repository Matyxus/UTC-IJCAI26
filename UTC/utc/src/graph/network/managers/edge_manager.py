...
class EdgeManager:
    ...
    def check_edge_sequence(self, sequence: List[Union[int, str, Edge]]) -> bool:
        ...
        if None in edges or not edges:
            return False
        # Check if edge sequence is correct
        current: Edge = edges.pop(-1)
        while edges:
            edge: Edge = edges.pop(-1)
            # print(f"Current edge: {current.info(False)}, edge: {edge.info(False)}")
            if current.get_id() not in self.edge_connections:
                print(f"Invalid sequence: {[edge.id for edge in edges]}, {current.id} does not have any incoming edges!")
                return False
            elif edge.get_id() not in self.edge_connections[current.id]:
                print(f"Invalid sequence: {[edge.id for edge in edges]}, {edge.id} is not incoming of: {current.id} !")
                return False
            current = edge
        return True
