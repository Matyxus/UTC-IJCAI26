...
class JunctionManager:
    ...
    def get_edges_connections(
            self, junctions: Iterable[Union[str, int, Junction]] = None,
            internal: bool = False
        ) -> Union[Dict[str, Set[str]], Dict[int, Set[int]]]:
        """
        ...
        :param internal: True if internal id's should be used, False otherwise
        ...
        def unpack(in_route: Optional[Route], out_route: Optional[Route]) -> Dict[str, Set[str]]:
            ...
            for index in range(len(edge_sequence) - 1):
                to_edge_id: str = edge_sequence[index+1].get_id(internal)
                from_edge_id: str = edge_sequence[index].get_id(internal)
