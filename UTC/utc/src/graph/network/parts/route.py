...
class Route(XmlObject):
    ...
    def get_length(self) -> float:
        ...
        if not self.is_valid():
            return -1
        return sum([edge.length for edge in self.edge_list])
    ...
    def travel(self) -> Tuple[float, str]:
        """
        :return: Tuple containing travel time of route and its destination (as junction id)
        """
        return self.get_travel_time(), self.get_destination()
