...
class Vehicle(XmlObject):
    ...
    def __init__(self, attributes: Dict[str, str], internal_id: int = -1):
        """
        :param attributes: time of vehicle
        :parm internal_id: vehicle internal id (by default increased by 1 on class creation)
        """
        assert("id" in attributes)
        super().__init__(
            "vehicle", attributes["id"],
            Vehicle._counter if internal_id == -1 else internal_id,
            attributes
        )
        if internal_id != -1:
            Vehicle._counter += 1
